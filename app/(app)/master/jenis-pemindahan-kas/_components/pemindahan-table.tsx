...
import { createJenisPemindahan, updateJenisPemindahan, toggleJenisPemindahanAktif } from "@/app/actions/master"
...
function PemindahanForm({ row, onDone }: { row: Row | null; onDone: () => void }) {
  ...
  function onSubmit(values: FormValues) {
    startTransition(async () => {
      const result = row
        ? await updateJenisPemindahan(row.id, values)
        : await createJenisPemindahan(values)
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1.5">
        ...
        <Input {...register("kode")} placeholder="JPK-01" className="bg-white/5 border-white/10 text-white" />
        {errors.kode && <p className="text-xs text-red-400">{errors.kode.message}</p>}
      </div>
      <div className="flex flex-col gap-1.5">
        <Label className="text-white/60 text-xs">Nama</Label>
        <Input {...register("nama")} placeholder="Nama jenis pemindahan" className="bg-white/5 border-white/10 text-white" />
...
export function PemindahanTable({ data }: { data: Row[] }) {
  return (
    <MasterTable
      data={data}
      dialogTitle="Jenis Pemindahan Kas"
      columns={[
        { key: "kode", label: "Kode" },
        { key: "nama", label: "Nama" },
      ...
      form={(row, onDone) => <PemindahanForm row={row as Row | null} onDone={onDone} />}
      onToggleAktif={toggleJenisPemindahanAktif}
    />
  )
}
